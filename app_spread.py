# app_spread.py · v7 (Funcionalidades Restauradas e Otimizadas)
# --------------------------------------------------------------------
# • Funcionalidades da v3 (adjust_complex_formula, DRE, DFC) restauradas.
# • Correção definitiva na atualização de Fórmulas Complexas.
# • Correção na lógica de destaque para valores que eram zero (Amarelo).
# • Mantida a otimização de performance para evitar lentidão/travamentos.
# • Mantido o relatório detalhado de alterações.
            ...
            with xw.App(visible=False) as app:
                with app.books.open(str(orig_tratada)) as wb:
                    for sht in wb.sheets:
                        if not sht.api.Visible == -1: continue # xlSheetVisible
...
def processar(
        ...
        try:
            with xw.App(visible=False) as app:
                with app.books.open(str(spr)) as wb:
                    sht = wb.sheets[0]
                    get_val = lambda r, c: sht.cells(r, c).formula_or_value
                    def set_val(r, c, v): sht.cells(r, c).value = v
                    ...
                    if (v_dfc := inserir_depreciacao_dfc(dfc_df, sht, dst_idx + 1, 199, atual, True)):
                        used_changed.add(v_dfc) # Adiciona como 'alterado'
    ...
    if not processed_ok:
        ...
        def set_val(r, c, v): ws.cell(r, c).value = v
        report, used_changed, used_new = atualizar_ws(get_val, set_val, abas, src_idx, dst_idx, atual, ant, start_row)
